...
spans = defaultdict(lambda: (defaultdict(lambda: [9999999, -1]), defaultdict(lambda: [])))
...
for xmlname in sys.argv[1:]:
    ...
    for line in xmlfile:
        ...
        if match:
            ...
            match = re.match(r"<link.*xtargets=\"([0-9 ]*);([0-9 ]*)\"", line)
            if match and match.group(src_idx+1):
                # Find the src
                src_arr = [int(x) for x in match.group(src_idx+1).split(" ")]
                src_min = min(src_arr)
                src_max = max(src_arr)
                curr_spans0, curr_spans1 = spans[currname]
                for src in src_arr:
                    curr_spans0[src] = (min(src_min, curr_spans0[src][0]),
                                        max(src_max, curr_spans0[src][1]))
                if match.group(2-src_idx):
                    # Create the target range
                    trg_arr = [int(x) for x in match.group(2-src_idx).split(" ")]
...
# Create the mapping
for filename, (src_links, trg_links) in spans.items():
    # Expand the links
    ...
    for i in range(len(block_ranges[0])):
        print("\t".join([("%d-%d" % (x[i][0], x[i][1]) if (i in x) else "") for x in block_ranges]))
    print("")
